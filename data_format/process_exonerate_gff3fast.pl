...
process_exonerate_gff3fast -t EST file1.e2g.EXONERATE file2.e2g.EXONERATE > out.gff3
...
Turns EXONERATE gff output into GFF3 for Gbrowse use.
...
while(<>) {
	...
	if( $state == 2 ) {
		...
		if( $ptag eq 'gene' ) {
			...
			$gene_name =~ s/\-\d+$//;
			$lastvulgar =~ s/cneo_TIGR:(\S+)\-\d+/cneo_TIGR:$1/;
		    }
		    if( $counter{$gene_name}++ ) {
			$gene_name .= "-$counter{$gene_name}";
		    }
		    my ($hitname) = split(/\s+/,$lastvulgar);
		    my $genelen = $lengths{$hitname};
		    my $chunk = $alnlens{$hitname};
		    if( ! $genelen ) { 
			warn("no genelen for $hitname\n");
		    } else {			
			$fields{'Length'}   = $genelen;
			$fields{'Coverage'} = int(100 * ($chunk/$genelen));
		    }
		...
		my $grouping_name = ($ptag =~ /gene|mRNA|match/) ? 'ID' : 'Parent';
		if( length($type) ) {
		...
		for my $master ( qw(ID Parent Target) ) {
		    if( defined $fields{$master} ) {
			unshift @fields, $master;
			last;
		    }
		}
		if(exists $fields{'vulgar'} ) {
		    $fields{'vulgar'} =~ s/\s/+/g;
		}
